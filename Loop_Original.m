...
%     P.NodeColor = colori(x,:);
%     pause(0.01)
            if(iter-iter_last_change >= 50 && iter>=iter_min)
                convergenza = true;
            end
%             C_hat = 1-abs(alfa-sum(x)/N);
%             A_hat = sum(abs((x)-(xinit)))/N;
...
legend('boxoff')
print(['C_Loop_Original(0,',str(3:end),')'],'-dpng','-r300')
figure()
...
legend('Original Model','Location','Best')
legend('boxoff')
print(['A_Loop_Original(0,',str(3:end),')'],'-dpng','-r300')
save(['Loop_Original(0,',str(3:end),').mat'])